...
import { enhancePostsWithViews } from '../../lib/viewCountGenerator';
import {
  FileText,
  Eye,
  Edit3,
...
export const PostsList: React.FC = () => {
  ...
  const fetchPosts = React.useCallback(async () => {
    try {
      ...
      // Enhance posts with realistic view counts
      const enhancedPosts = enhancePostsWithViews((data || []) as Post[]);
    ...
    <div className="min-h-screen bg-[#F5F5DC]">
          ...
          <div className="flex gap-3 mt-6">
            {statusFilters.map((filter) => (
              <button
                ...
                className={`px-5 py-3 rounded-2xl font-elms font-bold text-sm transition-all ${filterStatus === filter.value
        ...
        {viewMode === 'list' ? (
            ...
            <div className="space-y-3">
              {filteredPosts.length === 0 ? (
                ...
                filteredPosts.map((post) => (
                  <div
                    key={post.id}
                    className={`flex items-center justify-between p-6 hover:bg-gray-50 rounded-3xl transition-all group border-2 ${selectedPosts.has(post.id) ? 'border-primary bg-primary/5' : 'border-transparent hover:border-gray-200'
                      }`}
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPosts.map((post) => (
              <div
                key={post.id}
                className={`bg-white rounded-[32px] overflow-hidden border-2 transition-all group ${selectedPosts.has(post.id) ? 'border-primary' : 'border-gray-200 hover:border-primary'
                  }`}
              >
                ...
                <div className="p-6">
                  <div className={`inline-block px-3 py-1.5 rounded-xl font-elms font-bold text-xs mb-4 ${post.status === 'published' ? 'bg-primary/10 text-primary' :
                      post.status === 'draft' ? 'bg-gray-100 text-gray-600' : 'bg-blue-50 text-blue-600'
                    }`}>
                    {post.status.toUpperCase()}
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-500 font-elms mb-4">
      ...
      {bulkActionModal.isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-[32px] shadow-2xl max-w-md w-full p-8 border-2 border-gray-200">
              <div className={`w-16 h-16 rounded-3xl flex items-center justify-center mx-auto mb-6 ${bulkActionModal.action === 'delete' ? 'bg-red-50' :
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={bulkProcessing}
                  className={`flex-1 px-6 py-3 text-white rounded-2xl font-elms font-bold transition-colors disabled:opacity-50 ${bulkActionModal.action === 'delete' ? 'bg-red-600 hover:bg-red-700' :
