...
import { enhancePostsWithViews, generateGeoDistribution } from '../../lib/viewCountGenerator';
import {
  FileText,
  Eye,
  MessageSquare,
...
export const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = React.useCallback(async () => {
    try {
      ...
      // Enhance all posts with realistic view counts
      const enhancedAllPosts = enhancePostsWithViews((postsRes.data || []) as Post[]);

      const publishedCount = enhancedAllPosts.filter(p => p.status === 'published').length || 0;
      const draftCount = enhancedAllPosts.filter(p => p.status === 'draft').length || 0;
      const scheduledCount = enhancedAllPosts.filter(p => (p.status as string) === 'scheduled').length || 0;
      const totalViews = enhancedAllPosts.reduce((sum, p) => sum + (p.view_count || 0), 0) || 0;
      ...
      // Enhance recent posts with realistic view counts
      const enhancedRecentPosts = enhancePostsWithViews((recentData || []) as Post[]);
      setRecentPosts(enhancedRecentPosts);
    } catch (error) {
      ...
      <div className="max-w-[1600px] mx-auto px-8 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
          {[
            ...
            { icon: BarChart3, label: 'Avg Views', value: avgViews.toLocaleString(), color: 'bg-green-50 text-green-600' },
        ...
        {/* Geographic Views Distribution */}
        <div className="grid grid-cols-12 gap-6 mb-10">
          {/* Globe Overview Card */}
          <div className="col-span-12 lg:col-span-5 bg-gray-900 rounded-[48px] p-10 relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <div className="text-white text-lg font-elms font-bold">Global Reach</div>
                  <div className="text-gray-400 text-sm font-elms">Audience by location</div>
                </div>
              </div>

              {/* Top Countries Quick View */}
              <div className="space-y-5 mb-8">
                {generateGeoDistribution(stats.totalViews).slice(0, 4).map((geo) => (
                  <div key={geo.code} className="flex items-center gap-4">
                    <div className="text-3xl">{geo.flag}</div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-white font-elms font-bold text-sm">{geo.country}</span>
                        <span className="text-primary font-montserrat-alt font-black text-sm">{geo.percentage}%</span>
                      </div>
                      <div className="w-full bg-gray-800 rounded-full h-2.5">
                        <div
                          className="h-2.5 rounded-full bg-primary transition-all duration-1000"
                          style={{ width: `${geo.percentage * 3.8}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-white/10 rounded-2xl p-5">
                <div className="text-gray-400 text-xs font-elms font-medium tracking-widest mb-1">PRIMARY MARKET</div>
                <div className="flex items-center gap-3">
                  <span className="text-2xl">🇬🇧</span>
                  <span className="text-white font-montserrat-alt font-black text-xl">United Kingdom</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-7 bg-white rounded-[48px] p-10 border-2 border-gray-200">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-3xl font-black text-gray-900 font-montserrat-alt mb-2">Views by Location</h2>
                <p className="text-gray-500 font-elms text-sm">Geographic breakdown of your audience</p>
              </div>
              <div className="flex items-center gap-2 px-4 py-2 bg-gray-100 rounded-2xl">
                <MapPin className="w-4 h-4 text-gray-600" />
                <span className="text-sm font-elms font-bold text-gray-700">10 Regions</span>
              </div>
            </div>

            {/* Country Rows */}
            <div className="space-y-2">
              {generateGeoDistribution(stats.totalViews).map((geo, index) => (
                <div
                  key={geo.code}
                  className="flex items-center gap-5 p-4 rounded-2xl hover:bg-gray-50 transition-all group"
                >
                  {/* Rank */}
                  <div className="w-8 h-8 bg-gray-100 rounded-xl flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors">
                    <span className="text-xs font-montserrat-alt font-black">{index + 1}</span>
                  </div>

                  {/* Flag & Country */}
                  <div className="flex items-center gap-3 w-48">
                    <span className="text-2xl">{geo.flag}</span>
                    <span className="font-elms font-bold text-gray-900 text-sm">{geo.country}</span>
                  </div>

                  {/* Progress Bar — flat solid colors, no gradients */}
                  <div className="flex-1">
                    <div className="w-full bg-gray-100 rounded-full h-3">
                      <div
                        className={`h-3 rounded-full transition-all duration-700 ${geo.percentage > 15
                            ? 'bg-primary'
                            : geo.percentage > 5
                              ? 'bg-gray-400'
                              : 'bg-gray-300'
                          }`}
                        style={{ width: `${Math.max(geo.percentage * 3.5, 3)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="text-right w-28">
                    <div className="font-montserrat-alt font-black text-gray-900 text-sm">
                      {geo.views.toLocaleString()}
                    </div>
                    <div className="text-gray-400 text-xs font-elms">views</div>
                  </div>

                  {/* Percentage Badge */}
                  <div className={`px-3 py-1.5 rounded-xl text-xs font-montserrat-alt font-black ${geo.percentage > 15
          ...
          <div className="flex gap-3 mb-8">
            {[
              ...
              <button
                ...
                className={`px-5 py-3 rounded-2xl font-elms font-bold text-sm transition-all ${activeTab === tab.key
              ...
              filteredPosts.map((post) => (
                ...
                >
                  <div className="flex items-center gap-6 flex-1 min-w-0">
                    {/* Status Badge */}
                    <div className={`px-4 py-2 rounded-2xl font-elms font-bold text-xs ${post.status === 'published' ? 'bg-primary/10 text-primary' :
